...
const BulkChangeOccupationModal = ({ selectedCount, onClose, onConfirm, isLoading }) => {
  ...
  const occupationsList = occupationsData?.data?.results || occupationsData?.data || [];

  const handleSubmit = (e) => {
    e.preventDefault();
    if (selectedOccupation) {
      onConfirm(selectedOccupation);
    }
  };
        ...
        <form onSubmit={handleSubmit}>
            ...
            <div className="bg-amber-50 border border-amber-200 rounded-lg p-3 flex items-start space-x-2">
              ...
              <div className="text-sm text-amber-800">
                ...
                <ul className="list-disc list-inside mt-1 space-y-1">
                  ...
                  <li>Candidates must match the occupation's registration category</li>
                </ul>
              </div>
            </div>
            ...
            <div>
                ...
                >
                  ...
                  {occupationsList.map((occ) => (
                    <option key={occ.id} value={occ.id}>
                      {occ.occ_name} ({occ.occ_code}) - {occ.occ_category === 'workers_pas' ? "Worker's PAS" : 'Formal'}
                    </option>
                  ))}
                </select>
              )}
              {!loadingOccupations && (
                <p className="text-xs text-gray-500 mt-1">
                  {occupationsList.length} occupation(s) available
                </p>
              )}
            </div>
          </div>
          ...
          <div className="flex items-center justify-end space-x-3 px-6 py-4 border-t bg-gray-50 rounded-b-lg">
            ...
            <Button
              ...
              variant="primary"
              loading={isLoading}
              disabled={isLoading || !selectedOccupation}
            >
              Change Occupation
            </Button>
          </div>
...
export default BulkChangeOccupationModal;
