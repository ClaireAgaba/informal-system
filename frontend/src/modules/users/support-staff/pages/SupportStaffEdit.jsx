...
const SupportStaffEdit = () => {
  const { id } = useParams();
  const navigate = useNavigate();
  const queryClient = useQueryClient();
  // id is undefined when route is /users/support-staff/new (no :id param)
  ...
  useEffect(() => {
    ...
    if (!isNew && !isValidId) {
      toast.error('Invalid support staff ID');
      navigate('/users/support-staff');
      return;
    }
  ...
  useEffect(() => {
    if (supportStaff) {
      reset({
        ...
        contact: supportStaff.contact,
        department: supportStaff.department || '',
        account_status: supportStaff.account_status,
      });
    }
  }, [supportStaff, reset]);

  const updateMutation = useMutation({
    mutationFn: (data) => userApi.supportStaff.update(id, data),
    onSuccess: () => {
      queryClient.invalidateQueries(['support-staff']);
      toast.success('Support staff member updated successfully!');
      navigate(`/users/support-staff/${id}`);
    },
    onError: (error) => {
  ...
  const createMutation = useMutation({
    mutationFn: (data) => userApi.supportStaff.create(data),
    onSuccess: (response) => {
      queryClient.invalidateQueries(['support-staff']);
      toast.success('Support staff member created successfully!');
      navigate(`/users/support-staff/${response.data.id}`);
    },
    onError: (error) => {
    ...
    <div className="p-6">
      <div className="mb-6">
        <Button
          variant="outline"
          size="sm"
          onClick={() => navigate(isNew ? '/users/support-staff' : `/users/support-staff/${id}`)}
          className="mb-4"
        >
        ...
        <h1 className="text-2xl font-bold text-gray-900">
          {isNew ? 'Add New Support Staff Member' : 'Edit Support Staff Member'}
        </h1>
      </div>
      ...
      <form onSubmit={handleSubmit(onSubmit)}>
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          <div className="lg:col-span-2">
            <Card>
              <Card.Header>
                <h3 className="text-lg font-semibold text-gray-900">Support Staff Details</h3>
              </Card.Header>
            ...
            <Card>
              ...
              <Card.Content className="space-y-3">
                ...
                >
                  ...
                  {isNew ? 'Create Support Staff' : 'Save Changes'}
                </Button>
...
export default SupportStaffEdit;
