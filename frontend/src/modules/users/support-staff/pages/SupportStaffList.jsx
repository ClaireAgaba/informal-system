...
const SupportStaffList = () => {
  ...
  const { data, isLoading, error } = useQuery({
    queryKey: ['support-staff', page, searchTerm, statusFilter],
    queryFn: () =>
      userApi.supportStaff.getAll({
        page,
        page_size: pageSize,
        search: searchTerm,
  ...
  const supportStaff = data?.data?.results || [];
    ...
    <div className="p-6">
      <div className="mb-6">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-2xl font-bold text-gray-900">Support Staff Members</h1>
            <p className="text-gray-600 mt-1">Manage support staff like interns</p>
          </div>
          <Button variant="primary" size="md" onClick={() => navigate('/users/support-staff/new')}>
          ...
          <table className="w-full">
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {isLoading ? (
                ...
                <tr><td colSpan="7" className="px-6 py-4 text-center text-gray-500">No support staff found</td></tr>
              ) : (
                supportStaff.map((member) => (
                  <tr
                    key={member.id}
                    className="hover:bg-gray-50 cursor-pointer"
                    onClick={() => navigate(`/users/support-staff/${member.id}`)}
                  >
                    ...
                    <td className="px-6 py-4 text-right" onClick={(e) => e.stopPropagation()}>
                      <div className="flex items-center justify-end space-x-2">
                        <button onClick={() => navigate(`/users/support-staff/${member.id}`)} className="text-gray-600 hover:text-primary-600">
                          <Eye className="w-4 h-4" />
                        </button>
                        <button onClick={() => navigate(`/users/support-staff/${member.id}/edit`)} className="text-gray-600 hover:text-primary-600">
