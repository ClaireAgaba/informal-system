...
class FormalResultViewSet(viewsets.ViewSet):
    ...
    def transcript_pdf(self, request):
        ...
        def onFirstPage(canvas, doc):
            canvas.saveState()
            # Draw QR Code on right side, middle area (aligned with bio data section)
            qr_buffer.seek(0)
            canvas.drawImage(ImageReader(qr_buffer), A4[0] - 2.5*cm, A4[1] - 12*cm, width=1.8*cm, height=1.8*cm)
